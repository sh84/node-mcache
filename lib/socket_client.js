...
class SocketClient {
  /**
   * @param {Object} params
   * @param {String} [params.socket_path] - used unix socket path
   * @param {String} [params.pid_file_path] - used pid file path
  ...
  command(params, callback, second_try = false) {
    this.send_fn(params, (send_err, val) => {
      if (send_err && (
        send_err == 'connection closed' || send_err.code == 'EPIPE' || send_err.code == 'ECONNRESET'
      ) && !second_try) {
