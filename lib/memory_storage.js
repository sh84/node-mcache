...
class MemoryStorage {
   ...
   * @param {Number} [params.gc_interval_time=10] - Time between starts of gc iterations, ms
   */
  constructor(params) {
    ({
      ttl: this.ttl,
      gc_start: this.gc_start,
      gc_time: this.gc_time,
      gc_count: this.gc_count = 10000,
      gc_interval_time: this.gc_interval_time = 10
    } = params);
  ...
  _gc(keys_iterator, now, callback) {
    ...
    if (next.done) {
      ...
      setTimeout(() => this._gc(keys_iterator, now, callback), this.gc_interval_time);
    }
  }
